...
def get_max_Positon(aim):#获取最大
    ...
    if(pax>pin):
        ...
        p=pin
    return p
def entre_ACBM(alpha,beta,x,w,v,s,name):
    s_aim = s
    w_aim = w
    v_aim = v
    ...
    flag_max_s = 1.0
    flag_max_w = 1.0
    while (((flag_max_v)>aim_value or ((flag_max_s)>aim_value ) or (flag_max_w)>aim_value or (ii<=100 ) )and ii<=150 ):
        s_past = s_aim
...
def ACBM(k,xp,dataset_name):
    ...
    alpha = beta = 1.0
    w_a, s_a, v_a ,flag_list,flag_aim_list,flag_w_list,flag_s_list,flag_v_list= entre_ACBM(alpha, beta, xp, w, v, s,dataset_name)
    ...
    return  w_a, s_a, v_a ,flag_list,flag_aim_list,flag_w_list,flag_s_list,flag_v_list
...
if __name__ == '__main__':
    ...
    x=np.mat(z)
    w_a, s_a, v_a, flag_list, flag_aim_list, flag_w_list, flag_s_list, flag_v_list =  ACBM(k, x, dataset_name3)
